...
function openPopup() {
  ...
  function sendToWhatsApp() {
    ...
    let mensagem = `Olá! Quero baixar o sistema.%0A
    📌 CNPJ: ${cnpj}%0A
    🏢 Razão Social: ${razao}%0A
    📧 Email: ${email}`;

    // Seu número de WhatsApp (coloque o seu correto)
    let numero = "5575988808146";

    window.open(`https://wa.me/${numero}?text=${mensagem}`, "_blank");
  }
      function toggleMobileMenu() {
      var nav = document.getElementById('navmenu');
      nav.classList.toggle('navmenu-mobile');
    }
    function openPopup() {
      document.getElementById('popupForm').style.display = 'flex';
    }
    ...
    function sendToWhatsApp() {
      // Seu código de envio para WhatsApp
      closePopup();
    }